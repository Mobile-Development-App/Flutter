...
class AnalyticsWorkerService {
  ...
  Future<_WorkerResponse> _request(_WorkerOp op, Object payload) async {
    ...
    return c.future.timeout(
      ...
      // BUG FIX: sin onTimeout, el Completer queda en _pending para siempre si
      // el worker no responde a tiempo (memory leak + uso incorrecto posterior).
      // Al disparar el timeout eliminamos la entrada aquí mismo.
      onTimeout: () {
        _pending.remove(id);
        throw TimeoutException(
          'AnalyticsWorker: op ${op.name} (id=$id) timed out after 15s',
          const Duration(seconds: 15),
        );
      },
    );
