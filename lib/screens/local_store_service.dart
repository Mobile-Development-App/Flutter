...
class LocalStoreService {
  ...
  Map<String, dynamic>? _migrate(
    ...
    if (from <= 0) return data; // datos sin versión: tratar como compatibles.

    // BUG FIX: antes se retornaba null para cualquier gap de versión no
    // manejado, borrando silenciosamente todos los datos guardados al subir
    // el schemaVersion. Ahora registramos un warning y devolvemos los datos
    // tal cual (forward-compatible), en vez de descartarlos.
    // Agrega casos explícitos cuando introduzcas cambios incompatibles:
    // if (from == 1 && to == 2) { return _migrateV1toV2(data); }
    debugPrint(
